...
class TunnelCtl extends \Nethgui\Controller\Table\AbstractAction
{
    ...
    public function __construct($identifier = NULL)
    {
        if ($identifier !== 'enable' && $identifier !== 'disable' ) {
            throw new \InvalidArgumentException(sprintf('%s: module identifier must be one of "enable", "disable".', get_class($this)), 1499259829 );
        }
    ...
    public function bind(\Nethgui\Controller\RequestInterface $request)
    {
        ...
        if ( ! $service) {
            throw new \Nethgui\Exception\HttpException('Not found', 404, 1499259841);
        }
    ...
    {
        $this->setViewTemplate('NethServer\Template\IpsecTunnels\TunnelCtl');
        ...
        $this->getPlatform()->signalEvent('nethserver-ipsec-tunnels-save', array($this->parameters['service']));
    }
